        ...
        if (!disabled) {
          dayWidget = GestureDetector(
            ...
            onTap: () {
              ...
              for (var item in selectedDates) {
                if (item.compareTo(dayToBuild) == 0) {
                  isRemoved = true;
                }
              }
              if (!isRemoved) {
...
class _MonthPickerState extends State<MonthPicker>
  ...
  void didUpdateWidget(MonthPicker oldWidget) {
    ...
    if (widget.selectedDates != oldWidget.selectedDates) {
      final int monthPage =
          _monthDelta(widget.firstDate, widget.selectedDates[0]);
      _dayPickerController = PageController(initialPage: monthPage);
      _handleMonthPageChanged(monthPage);
    }
  }
  ...
  void _handleModeChanged(DatePickerMode mode) {
    ...
    setState(() {
      ...
      if (_mode == DatePickerMode.day) {
        SemanticsService.announce(
            localizations.formatMonthYear(_selectedDates[0]), textDirection);
      } else {
        SemanticsService.announce(
...
///
/// See [showListDatePicker].
// typedef SelectableDayPredicate = bool Function(DateTime day);

/// Shows a dialog containing a material design date picker.
///
/// The returned [Future] resolves to a list of date selected by the user when the
...
///
/// ```dart
/// Future<List<DateTime>> selectedDate = showListDatePicker(
///   context: context,
...
///    a year.
Future<List<DateTime>> showListDatePicker({
